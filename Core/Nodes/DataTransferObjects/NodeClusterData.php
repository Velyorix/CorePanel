...
readonly class NodeClusterData
{
    public function __construct(
        ...
        public NodeClusterStatus $status = NodeClusterStatus::Active,
    ...
    public static function fromArray(array $data): self
    {
        ...
        if ($key === '') {
            throw new InvalidArgumentException('The node cluster key is required.');
        }

        if (! preg_match('/^[a-z0-9]+(?:[_-][a-z0-9]+)*$/', $key)) {
            throw new InvalidArgumentException("Invalid node cluster key [{$key}].");
        }

        $statusValue = self::nullableString($data['status'] ?? null) ?? NodeClusterStatus::Active->value;
        $status = NodeClusterStatus::tryFrom($statusValue);

        if ($status === null) {
            throw new InvalidArgumentException("Invalid node cluster status [{$statusValue}].");
        }
