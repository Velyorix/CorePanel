<?php

namespace Core\Billing\Services;

use Core\Billing\Enums\QuoteStatus;
use Core\Billing\Models\BillingSequence;
use Core\Billing\Models\Quote;
use Illuminate\Support\Facades\DB;
use InvalidArgumentException;
use RuntimeException;

/**
 * Configurable quote numbering.
 * Assigns numbers only — does not change quote status.
 */
class QuoteNumberService
{
    public const SEQUENCE_NAME = 'quote';

    public function __construct(
        private readonly BillingSettings $billingSettings,
    ) {
    }

    /**
     * Assign the next configured quote number.
     * Idempotent when quote_number is already set.
     */
    public function assignNumber(Quote $quote): Quote
    {
        return DB::transaction(function () use ($quote): Quote {
            $locked = Quote::query()
                ->whereKey($quote->id)
                ->lockForUpdate()
                ->firstOrFail();

            if (filled($locked->quote_number)) {
                return $locked;
            }

            if ($locked->status !== QuoteStatus::Draft) {
                throw new InvalidArgumentException(
                    'Only draft quotes can receive a quote number.',
                );
            }

            $number = $this->nextFormattedNumber();

            $locked->forceFill([
                'quote_number' => $number,
            ])->save();

            return $locked->fresh(['items', 'client']) ?? $locked;
        });
    }

    public function previewNext(): string
    {
        $sequence = BillingSequence::query()
            ->where('name', self::SEQUENCE_NAME)
            ->first();

        $year = (int) now()->format('Y');
        $nextValue = 1;

        if ($sequence !== null) {
            $nextValue = $this->shouldReset($sequence, $year)
                ? 1
                : ((int) $sequence->current_value) + 1;
        }

        return $this->format($nextValue, $year);
    }

    private function nextFormattedNumber(): string
    {
        $sequence = BillingSequence::query()
            ->where('name', self::SEQUENCE_NAME)
            ->lockForUpdate()
            ->first();

        if ($sequence === null) {
            $sequence = BillingSequence::query()->create([
                'name' => self::SEQUENCE_NAME,
                'current_value' => 0,
                'year' => (int) now()->format('Y'),
            ]);

            $sequence = BillingSequence::query()
                ->whereKey($sequence->id)
                ->lockForUpdate()
                ->firstOrFail();
        }

        $year = (int) now()->format('Y');

        if ($this->shouldReset($sequence, $year)) {
            $sequence->forceFill([
                'current_value' => 0,
                'year' => $year,
            ])->save();
        }

        $next = ((int) $sequence->current_value) + 1;

        if ($next < 1) {
            throw new RuntimeException('Quote sequence produced an invalid value.');
        }

        $sequence->forceFill([
            'current_value' => $next,
            'year' => $year,
        ])->save();

        return $this->format($next, $year);
    }

    private function shouldReset(BillingSequence $sequence, int $year): bool
    {
        if (! (bool) config('corepanel.billing.quote_numbering.reset_yearly', true)) {
            return false;
        }

        return $sequence->year === null || (int) $sequence->year !== $year;
    }

    private function format(int $value, int $year): string
    {
        $prefix = $this->billingSettings->quotePrefix();
        $separator = (string) config('corepanel.billing.quote_numbering.separator', '-');
        $padding = max(1, (int) config('corepanel.billing.quote_numbering.padding', 6));
        $includeYear = (bool) config('corepanel.billing.quote_numbering.include_year', true);

        $parts = [$prefix];

        if ($includeYear) {
            $parts[] = (string) $year;
        }

        $parts[] = str_pad((string) $value, $padding, '0', STR_PAD_LEFT);

        return implode($separator, $parts);
    }
}
