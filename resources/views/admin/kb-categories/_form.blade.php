@php
    /** @var \Core\KnowledgeBase\Models\KbCategory|null $category */
    $category ??= null;
@endphp
...
<div class="grid gap-4 sm:grid-cols-2">
    ...
    <x-ui.select name="status" :label="__('Status')" required>
        @foreach ($statuses as $status)
            <option value="{{ $status->value }}" @selected((string) old('status', $category?->status?->value ?? \Core\KnowledgeBase\Enums\KbCategoryStatus::Active->value) === $status->value)>
                {{ $status->label() }}
            </option>
        @endforeach
    </x-ui.select>
